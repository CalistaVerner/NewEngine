...
/// Runtime telemetry (FPS, dt, fixed step info).
/// Не владеет логгером — использует `log` crate.
...
impl Telemetry {
    ...
    /// Включить/выключить логирование FPS
    /// `period_sec` — минимальный период между логами
    ...
    pub fn frame_tick(&mut self, dt: Duration, fixed_alpha: f32, fixed_tick: u64) {
        ...
        if elapsed >= self.fps_period_sec {
            ...
            log::info!(
                "[Telemetry] fps={:.1} dt_ms={:.2} fixed_alpha={:.2} fixed_tick={}",
                self.fps,
                self.dt_ms,
                self.fixed_alpha,
                self.fixed_tick
    ...
    pub fn record_scope(&mut self, _name: &'static str, _dur: Duration) {
        // Зарезервировано под future profiling (zones, tracy, chrome trace).
    }
}